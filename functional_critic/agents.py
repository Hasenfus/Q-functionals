...
from numpy.ma.core import inner, maximum_fill_value, shape
from functional_critic import utils, utils_for_q_learning, buffer_class
from functional_critic.cartesian_sum import cartesian_sum
import torch
...
class BaseAgent(nn.Module):
    ...
    def __init__(self, params, env, state_size, action_size, device, seed):
        ...
        assert numpy.allclose(self.env.action_space.high, self.max_a)
        
        self.act_dim = self.env.action_space.shape[0]
        
        self.L = (self.env.action_space.high[0] - self.env.action_space.low[0])
        ...
        if self.params["coefficient_scaling_exponent"] != 0:
            inverse_norm_scaling = torch.transpose(self.inner_arg_freq.unsqueeze(dim=0).to(torch.float32), 0, 1).norm(dim=[1, 2])
        ...
        self.visualization_state = torch.from_numpy(self.env.observation_space.sample()).to(self.device)
        ...
        with torch.no_grad():
            self._coefficient_module_1[-1].weight.data = self._coefficient_module_1[-1].weight.data * final_layer_init_scale
        ...
        if self.params['minq']:
            ...
            with torch.no_grad():
                self._coefficient_module_2[-1].weight.data = self._coefficient_module_2[-1].weight.data * final_layer_init_scale
                self._coefficient_module_2[-1].bias.data = self._coefficient_module_2[-1].bias.data * final_layer_init_scale
        ...
        if self.use_precomputed_basis:
            with torch.no_grad():
                ...
                precomputed_actions, precomputed_basis_representations = self._construct_precomputed_basis(self._num_precomputed)
    ...
    def quantile_schedule(self, step=-1):
        if self.params['anneal_quantile_sampling']:
            return self.params['quantile_sampling_percent'] - (self.params['quantile_sampling_percent'] * step / self.params['max_step'])
        else:
    ...
    def gaussian_sample_action_space(self, step, action_to_sample=None):
        if action_to_sample is None:
            action_to_sample = self.action_to_sample
        return torch.normal(0, self.std_gaussian_schedule(step)+1e-4, (action_to_sample, self.act_dim), device=self.device)
    ...
    def sample_action_space(self, step, action_to_sample=None):
        if self.params['sample_method'] == 'uniform':
            ...
            return self.gaussian_sample_action_space(step=(step if self.params['anneal_std'] else -1), action_to_sample=action_to_sample)
        else:
    ...
    def get_best_qvalue_and_action(self, s, step=-1, action_to_sample=None):
        ...
        best_action, best_q = self.compute_Q_star(s, return_best_actions=True, quantile_sampling=should_quantile_sample, step=step, action_to_sample=action_to_sample)
    ...
    def get_all_q_values_and_action_set(self, s, actions=None, prefetch_basis_shape=None):
        ...
        '''
        with torch.no_grad():
            coefficients = self.get_coefficients(self._coefficient_module_1, s) # [batch_size, num_coefficient_scale * inner_arg_freq]
            _, action_representations = self.get_actions_and_basis_representations(actions, prefetch_basis_shape=prefetch_basis_shape)
            coefficients = coefficients.unsqueeze(dim=1) # [256 x 1 x 50]
            output = action_representations.mul(coefficients) # [256 x 500 x 50] --> [batch_dim x num_actions_to_sample x self.rank**self.act_dim]
    ...
    def forward(self, s, a, coefficients=None):
        ...
        # s: [batch_dim x self.state_size]
        # a: [batch_dim x self.act_dim]
    ...
    def compute_Q_star(self, s, return_best_actions=False, quantile_sampling=False, step=-1, do_minq=False, action_to_sample=None):
        ...
        with torch.no_grad():
            ...
            actions, action_representations = self.get_actions_and_basis_representations(actions=actions, prefetch_basis_shape=prefetch_basis_shape)
            
            coefficients_1 = coefficients_1.unsqueeze(dim=1) # [256 x 1 x 50]
            output = action_representations.mul(coefficients_1) # [256 x 500 x 50] --> [batch_dim x num_actions_to_sample x self.rank**self.act_dim]
            ...
            if do_minq:
                ...
                catted_action_values = torch.cat((output.unsqueeze(-1), output_2.unsqueeze(-1)), dim=2) #[batch x num_sampled_actions x 2]
            ...
            if self.params['entropy_regularized'] > 0: 
                ...
                if output.shape[0] > 1:
                    ...
                    output = self.T*torch.logsumexp(output/self.T, dim=1) - self.T*torch.log(torch.tensor([action_to_sample]).to(torch.float32).to(self.device))
            ...
            if quantile_sampling and self.quantile_schedule(step) != 0:
                ...
                best_actions = torch.gather(actions, 1, best_indices.unsqueeze(dim=2).repeat(1, 1, self.act_dim)).reshape(-1, self.act_dim)
                ...
                best_actions = torch.gather(actions, 1, output.indices.reshape(-1, 1, 1).repeat(1, 1, self.act_dim)).reshape(-1, self.act_dim)
                ...
                if self.params['use_target_policy_smoothing']:
                    ...
                    if do_minq:
                        ...
                        catted_action_values = torch.cat((noisy_outputs.unsqueeze(-1), noisy_outputs_2.unsqueeze(-1)), dim=2)
    ...
    def update(self, target_Q, step=-1):
        ...
        Q_star = target_Q.compute_Q_star(sp_matrix, quantile_sampling=self.params['use_quantile_sampling_bootstrapping'], step=step, do_minq=self.params['minq'])
        ...
        if self.params.get("log_reward_function", None):
            update_param["fourier_coefficients"] = self.get_coefficients(self._coefficient_module_1, self.visualization_state)
...
class FourierAgent(BaseAgent):
    ...
    def __init__(self, params, env, state_size, action_size, device, seed):
        super(FourierAgent, self).__init__(params, env, state_size, action_size, device, seed)
    ...
    def  get_actions_and_basis_representations(self, actions=None, prefetch_basis_shape=None):
        ...
        if actions is not None:
            ...
            inner_arg_freq = torch.transpose(self.inner_arg_freq, dim0=0, dim1=1).to(self.device) # [act_dim x num_basis]
            ...
            sines_cat_cosines = torch.cat((sines, cosines), dim=-1) # [batch_dim, action_to_sample x (2*self.rank**self.act_dim)]
...
class PolynomialAgent(BaseAgent):
    ...
    num_coefficient_scale = 1 # The scale factor for different basis 
    def __init__(self, params, env, state_size, action_size, device, seed):
        super(PolynomialAgent, self).__init__(params, env, state_size, action_size, device, seed)
...
class LegendreAgent(BaseAgent):
    ...
    def __init__(self, params, env, state_size, action_size, device, seed):
        ...
        super(LegendreAgent, self).__init__(params, env, state_size, action_size, device, seed)
    ...
    def  get_actions_and_basis_representations(self, actions=None, prefetch_basis_shape=None):
        if actions is not None:
            ...
            indices = torch.repeat_interleave(unsqueezed_inner_arg_frequency, repeats=evaluated_1_d_legendre_lookup.shape[1], dim=1)
